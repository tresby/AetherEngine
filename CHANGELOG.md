...
- **PGS subtitles blank for tens of seconds after a seek or audio-track switch (#112).** PGS (Blu-ray) subtitles are stateful and sparse: a line stays on screen from its composition until the next one clears it, often tens of seconds later. The overlay side demuxer seeked to a fixed 2 s before the playhead, so a seek (or the producer restart an audio-track switch triggers) landed after the active line's composition and the line never reconstructed, showing nothing until the next dialogue line. The reader now seeks back a source-aware lead-in and reconstructs the active line in a single forward pass, letting the existing stale-arrival gate publish the composition whose window covers the playhead. On an indexed container (MP4/MKV) that scans the full window nearly for free; on a disc source (concat MPEG-TS with no index) the window is capped tight so a remote ISO is not re-downloaded. A first attempt scanned backward in geometric probe steps with a throwaway decoder; on a remote Blu-ray that had no index and where discard drops packets only after they are read off the wire, each probe re-downloaded its look-back span and a sparse region ran the scan to the 60 s cap (~100 s of disc re-read, un-cancellable), so the reader was superseded before it served a cue and no subtitles appeared at all.
