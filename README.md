...
```swift
.package(url: "https://github.com/superuser404notfound/AetherEngine", from: "3.4.1")
```
...
Pin `from: "3.4.1"` in your `Package.swift` to allow patch + minor updates while excluding breaking changes:

```swift
.package(url: "https://github.com/superuser404notfound/AetherEngine", from: "3.4.1")
```

Pin to `.upToNextMinor(from: "3.4.1")` for stricter teams that prefer to opt into minor bumps explicitly. See [CHANGELOG.md](CHANGELOG.md) for the per-release index.
